<?php 

session_start();

if (isset($_SESSION['id']) && isset($_SESSION['email'])) {

    $phoneID = $_POST['phoneID'];

    include_once 'dbConnect.php';

    $getsql = "SELECT * FROM mobiles where phoneID = $phoneID;";
    $result = mysqli_query($conn,$getsql);
    $resultCheck = mysqli_num_rows($result);
    $row = mysqli_fetch_assoc($result);
    $userSql = "SELECT * FROM users where userID = ".$row['userID'].";";
    $userResult = mysqli_query($conn,$userSql);
    $userRow = mysqli_fetch_assoc($userResult);
?>

<!DOCTYPE html>
<html>
    <head>

        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <link rel="shortcut icon" type="x-icon" href="icons\logo2.png">
        <link href="style.css" rel="stylesheet">
        <!-- CSS only -->
        <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-1BmE4kWBq78iYhFldvKuhfTAU6auU8tT94WrHftjDbrCEXSU1oBoqyl2QvZ6jIW3" crossorigin="anonymous">
        <!-- JavaScript Bundle with Popper -->
        <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-ka7Sk0Gln4gmtz2MlQnikT1wXgYsOg+OMhuP+IlRH9sENBO0LRn5q+8nbTov4+1p" crossorigin="anonymous"></script>
        
        <script type="text/javascript" src="Scripts/jquery-2.1.1.min.js"></script>
        <script type="text/javascript" src="Scripts/bootstrap.min.js"></script>

        <script src="product.js"></script>
        <title>Mobilezone</title>

    </head>
    <body class="bg-light">

        <div class="container">
            
            <nav class="navbar sticky-top navbar-light bg-light border-bottom border-2">
                <div class="container-fluid">

                    <a class="navbar-brand" href="product.php">
                        <img src="icons\logo1.png" alt="" width="65" height="40">
                        </a>
                    <ul class="nav justify-content-end">
                        <li class="nav-item">
                            <a class="nav-link link-dark active" href="product.php"><b>Home</b></a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link link-dark " href="sell.php"><b>Sell</b></a>
                        </li>
                         <li class="nav-item my-auto">
                                <form id="user" name="menuForm" method="POST" class="" action="productMenu.php">
                                    <select onChange="user.submit()" name="userSel" id="usersel" class="userButton bg-light">
                                        ><option hidden value="user"><?php echo $_SESSION['name']; ?></option>
                                        <option value="Dashbord">Dashbord</option>
                                        <option value="logout">Logout</option>
                                    </select>
                                </form>
                        </li>
                    </ul>

                </div>  
                
            </nav>

            <section class="container mt-md-2 mt-2">

                <div class="row">  
                    <div class="col-sm-10 mx-auto my-2">
                        <div class="row">
                            <div class="col col-md-8 mb-4 mb-md-0">
                                <img src="<?php echo $row['imageLocation']?>" class="card-img-top" alt="...">
                            </div>
                            <div class="card col-md-4">

                                <h5><?php echo $row['mobileModel']?></h5>
                                <p class="mb-2 text-muted text-uppercase small"><?php echo $row['brand']?></p>
                                <p><span class="mr-1"><strong>Rs <?php echo $row['price']?></strong></span></p>
                                <div class="table-responsive">
                                    <table class="table table-sm table-borderless mb-0">
                                        <tbody>
                                        <tr>
                                                <th class="pl-0 w-25" scope="row"><h6 class="card-text mt-1">Storage</h6></th>
                                                    <td><?php echo $row['storage'];?></td>
                                            </tr>
                                            <tr>
                                                <th class="pl-0 w-25" scope="row"><h6 class="card-text mt-1">RAM</h6></th>
                                                    <td><?php echo $row['ram'];?></td>
                                            </tr>
                                            <tr>
                                                <th class="pl-0 w-25" scope="row"><h6 class="card-text mt-1">Year</h6></th>
                                                    <td><?php echo $row['year'];?></td>
                                            </tr>
                                           
                                            <tr>
                                                <th class="pl-0 w-25" scope="row"><h6 class="card-text mt-1">Condition</h6></th>
                                                    <td><?php echo $row['condition'];?></td>
                                            </tr>
                                            <tr>
                                                <th class="pl-0 w-25" scope="row"><h6 class="card-text mt-1">Ownership</h6></th>
                                                    <td><?php echo $row['ownership'];?></td>
                                            </tr>
                                            <tr>
                                                <th class="pl-0 w-25" scope="row"><h6 class="card-text mt-1">Location</h6></th>
                                                    <td><?php echo $row['location'];?></td>
                                            </tr>
                                        </tbody>
                                    </table>
                                    <form method="POST" action="reviewUpload.php">
                                                    <button onclick="order.submit()" class="btn btn-secondary" align="right">Share</button>
                                                </form>
                                </div>
                            </div>
                        </div>
                        <div class="col-sm-12 my-2">
                            <div class="row">
                                <div class="card mt-1 col-12 p-2">
                                    <div class="row">
                                        <div class="col-12 col-sm-8">
                                            <h6 class="card-text mt-1">Description</h6>
                                            <p class="card-text"><?php echo $row['description']?></p>
                                            <div class="input-group rounded mb-1 mt-1">
                                                <form method="POST" action="order.php">
                                                    <input type="text" value="<?php echo $phoneID?>" hidden name="phoneID">
                                                    <button onclick="order.submit()" class="btn btn-secondary">Intrested</button>
                                                </form>
                                            </div>
                                        </div>
                                        <div class="col-12 col-sm-4 my-1">
                                            <h6 class="card-text mt-1">Posted By</h6>
                                            <div class="table-responsive">
                                                <table class="table table-sm table-borderless mb-0">
                                                    <tbody>
                                                        <tr>
                                                            <th class="pl-0 w-25" scope="row"><h6 class="card-text mt-1">Name</h6></th>
                                                                <td><?php echo $userRow['name'];?></td>
                                                        </tr>
                                                        <tr>
                                                            <th class="pl-0 w-25" scope="row"><h6 class="card-text mt-1">Contact</h6></th>
                                                                <td><?php echo $userRow['phone'];?></td>
                                                        </tr>
                                                    </tbody>
                                                </table>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>     
                </div>

            </section>
     </div>

    </body>
</html>

<?php

}else{

header("Location: loginForm.php");

exit();

}

?>