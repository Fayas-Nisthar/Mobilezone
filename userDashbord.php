...
if (isset($_SESSION['id']) && isset($_SESSION['email'])) {
    ...
    $phoneSql = "SELECT * FROM mobiles WHERE mobiles.userID=".$userID.";";
    $resultID = mysqli_query($conn,$phoneSql);
    $value = mysqli_fetch_array($resultID);
    
    if($value != 0){
    $userPhoneID = $value['phoneID'];
    }
     else{
         $userPhoneID = 0;
     }
    ...
    $getsql = "SELECT * FROM users,orders WHERE orders.userID !=" .$userID." AND orders.phoneID = ".$userPhoneID." AND orders.userID = users.userID;";
                        ...
                        <?php  
                            ...
                            ?>
                                <div class="card p-1">
                                    <div class="row">
                                        <div class="col">
                                            <p><?php echo $row['name']?> has intrest in buying your mobile. Contact: <?php echo $row['phone']?></p>
                                        </div> 
                                    </div>  
                                 </div>
                            <?php 
